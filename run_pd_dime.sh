...
if [ "$files" = "0" ]; then
  ...
  for PCALLER in ../../../results_peaks/*; do
    ...
    for PMODE in $PCALLER/$SET/*; do
      ...
      # in R 
      echo "library(DIME)" > $SCRIPT
      echo "library(edgeR)" >> $SCRIPT
      echo "cov <- read.table('coverage.txt')" >> $SCRIPT
      echo "cov\$id <- with(cov, paste0(V1,'_',V2,'_',V3))" >> $SCRIPT
      echo "tmmnorm <- cpm(cov[,c(4,5,6,7)], normalized.lib.sizes=TRUE)" >> $SCRIPT
      echo "cov\$S1 <- rowMeans(tmmnorm [,c(1,2)])" >> $SCRIPT
      echo "cov\$S2 <- rowMeans(tmmnorm [,c(3,4)])" >> $SCRIPT
      echo "cov\$logratio <- with(cov,log(S2/S1))" >> $SCRIPT
      echo "cov <- cov[is.finite(cov\$logratio),]" >> $SCRIPT #new
      echo "data <- list(cov\$logratio)" >> $SCRIPT
      echo "set.seed(1234)" >> $SCRIPT
      echo "test <- DIME(data,gng.max.iter=100,gng.rep=5,inudge.max.iter=100,inudge.rep=5,nudge.max.iter=100,nudge.rep=5)" >> $SCRIPT
      echo "best <- DIME.classify(data,test\$best)" >> $SCRIPT
      echo "cov\$isdiffo <- best\$class" >> $SCRIPT
      echo "cov\$fdr <- best\$fdr" >> $SCRIPT
      echo "cov\$fdr[is.null(cov\$fdr)] <- 1" >> $SCRIPT
      echo "res <- cbind(as.character(cov\$V1),cov\$V2,cov\$V3,cov\$fdr,cov\$logratio)" >> $SCRIPT
